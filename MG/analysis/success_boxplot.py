...
A game is run with varied memories for the agents.  Parameters for the game and agents are defined below.
Two plots measuring success by agent type (memory) are produced and saved with logging details.
"""
...
if __name__ == "__main__":
    ...
    payoff_cls = DollarGamePayoff     # or ScaledMGPayoff, SmallMinorityPayoff
    ...
    # --- Run simulation ---
    results = run_varied_memory_game(m_values, s, payoff_scheme=payoff_cls, N_per_m=N_per_m, rounds=rounds, lambda_value=lambda_value)
    ...
    for m, metrics in results.items():
        ...
        df_players = pd.DataFrame(rows, columns=["memory", "success_rate", "average_score", "strategy_switches"])
